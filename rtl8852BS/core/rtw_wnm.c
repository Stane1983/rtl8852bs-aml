...
u8 rtw_wmn_btm_rsp_reason_decision(_adapter *padapter, u8* req_mode)
{
	...
	if (recvinfo->signal_strength_data.avg_val >= pmlmepriv->roam_rssi_threshold) {
		reason = 1;
...
static void rtw_wnm_nb_info_update(
	u32 nb_rpt_entries, u8 from_btm, struct roam_nb_info *pnb,
	struct wnm_btm_cant *pcandidates, u8 *nb_rpt_is_same)
{
	u8 is_found;
	u32 i, j;
