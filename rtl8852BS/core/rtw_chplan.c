...
u8 init_channel_set(_adapter *padapter, u8 ChannelPlan, RT_CHANNEL_INFO *channel_set)
{
	...
	#if CONFIG_DFS
	rtw_phl_regulation_set_capability(GET_PHL_INFO(dvobj), (
	(b2_4GBand == _TRUE ?  CAPABILITY_2GHZ : 0) |
	(b5GBand == _TRUE ? CAPABILITY_5GHZ : 0) | CAPABILITY_DFS));
	#else /* !CONFIG_DFS */
	rtw_phl_regulation_set_capability(GET_PHL_INFO(dvobj), (
	(b2_4GBand == _TRUE ? CAPABILITY_2GHZ : 0) |
	(b5GBand == _TRUE ? CAPABILITY_5GHZ : 0)));
	#endif /* CONFIG_DFS */

	/*Query channel plan from PHL*/
	rtw_phl_regulation_query_chplan(GET_PHL_INFO(dvobj), REGULQ_CHPLAN_FULL, NULL, &plan);
	...
	for (i = 0; i < plan.cnt; i++) {
		...
		if (channel_set[chanset_size].flags & RTW_CHF_NO_IR) {
			if (rfctl->country_ent || (channel_set[chanset_size].flags & RTW_CHF_DFS))
				RTW_INFO("ch%u is PASSIVE%s\n", plan.ch[i].channel, plan.ch[i].property & CH_DFS ? " DFS" : "");
		}
...
#define COUNTRY_CHPLAN_ASSIGN_EN_11AX(_val) , .en_11ax = (_val)
#else
#define COUNTRY_CHPLAN_ASSIGN_EN_11AX(_val)
#endif
...
static const struct country_chplan country_chplan_map[] = {
	...
	COUNTRY_CHPLAN_ENT("DZ", 0x26, 1), /* Algeria */
	...
	COUNTRY_CHPLAN_ENT("KW", 0x47, 1), /* Kuwait */
	...
	COUNTRY_CHPLAN_ENT("UA", 0x36, 1), /* Ukraine */
...
bool rtw_get_chplan_from_country(const char *country_code, struct country_chplan *ent)
{
	char code[2];
	bool ret = _FALSE;
	struct rtw_regulation_country_chplan plan = {0};

	code[0] = alpha_to_upper(country_code[0]);
	code[1] = alpha_to_upper(country_code[1]);

#ifdef CONFIG_CUSTOMIZED_COUNTRY_CHPLAN_MAP
	/* ToDo: there is no customized country channel plan in PHL. */
#elif RTW_DEF_MODULE_REGULATORY_CERT
	/* ToDo: module certificated regulatory map */
#else
	if (rtw_phl_query_country_chplan(code, &plan)) {
		_rtw_memcpy(ent->alpha2, code, 2);
		ent->chplan = plan.domain_code;
		#ifdef CONFIG_80211AC_VHT
		ent->en_11ac = (plan.support_mode & BIT(4)) ? 1 : 0;
		#endif
		/* ToDo: CONFIG_80211AX_HE */
		ret = _TRUE;
	}
#endif
...
void dump_chplan_ver(void *sel, struct _ADAPTER *padapter)
{
	...
	if (rtw_phl_query_regulation_info(GET_PHL_INFO(d), &info))
