...
#ifdef PHL_DIS_PPDU_STS_RPT
	PHL_INFO("==> Default DISABLE RX_PPDU_STS for Band0\n");
	/* Disable PPDU STS in default for BAND-0 for phy status */
	hal->hal_com->band[HW_BAND_0].ppdu_sts_appen_info = 0;
	hal->hal_com->band[HW_BAND_0].ppdu_sts_filter = 0;
	rtw_hal_mac_ppdu_stat_cfg(hal, HW_BAND_0, false, 0, 0);
	phl_com->ppdu_sts_info.en_ppdu_sts[HW_BAND_0] = false;

	rtw_phl_init_ppdu_sts_para(phl_com, _FALSE, _FALSE, 0);
#else
